...
如果你经常折腾代理、跑号服务器、API 连通性，v6.5 这一版就很适合放在自己的机器上跑。它不是那种“我在别的机器上测通了，所以你那里也一定能用”的玄学榜单，而是直接回答一个更实用的问题：**你这台服务器能不能连上这个代理，并且通过它访问目标服务？**
...
- `zai`：检测 z.ai（自动跟随跳转到 chat.z.ai）和 Z.AI API 域名（api.z.ai）。z.ai 的边缘是阿里云 ESA 而非 Cloudflare，所以这里专门识别“代理侧注入的挑战页/拦截页”——这类响应状态码常为 200 却没有真实页面内容，不识别就会被误判为可达。该模式在同一轮内会多试一次，降低边缘偶发挑战造成的误判。
