...
如果你经常折腾代理、跑号服务器、API 连通性，v6.6 这一版就很适合放在自己的机器上跑。它不是那种“我在别的机器上测通了，所以你那里也一定能用”的玄学榜单，而是直接回答一个更实用的问题：**你这台服务器能不能连上这个代理，并且通过它访问目标服务？**
...
- `zai`：检测 z.ai（自动跟随跳转到 chat.z.ai）、Z.AI API 域名（api.z.ai）与 ZCode 套餐入口（zcode.z.ai；默认只打根路径，置 `ZAI_AUX_DEEP=1` 时改打无鉴权配置接口并校验特征串）。z.ai 的边缘是阿里云 ESA 而非 Cloudflare，所以这里专门识别“代理侧注入的挑战页/拦截页”——这类响应状态码常为 200 却没有真实页面内容，不识别就会被误判为可达。该模式在同一轮内会多试一次，降低边缘偶发挑战造成的误判。**zcode.z.ai 同时是本档的达标门槛**：它每轮都不通过时，线路即便服务与 API 全通也不算有效（`usable=false`，原因写进 `usable_reason`），因为对 ZCode 客户端而言这条线路用不了。
...
- 我的仓库按最新添加或更新置顶。
- 有效代理和我的仓库都支持标签筛选，例如等级、服务可达、API 域名可达、CF、国家、IP 类型，以及本档可用性与附加目标（如 ZCode 入口）可达性。
- 每个标签都有鼠标 title 说明，hover 一下就能看到“这标签到底是什么意思”。
- 「本档可用」（`usable`）与等级是两件事：等级说这条线路本身质量如何，可用性说它对本模式的目标服务到底能不能用。带达标门槛的模式（如 `zai`）在结果、仓库和订阅链接里都按可用性说话。
- 仓库条目还会记录连续通过轮次（`pass_streak`）与共享出口标记（`ip_shared`），用于筛掉「看着多条、其实同源」和「只通过过一次」的记录。
- 仓库链接使用浏览器稳定 token，公共部署时不同用户不会互相覆盖。
- 订阅链接支持可选过滤参数：`grade=A,B`、`usable=1`、`stable=1`、`proto=http,socks5`、`exclude_socks4=1`、`exclude_shared_ip=1`、`limit=50`、`comment=1`（给每行加 `# 元信息`）。不带参数时输出与从前逐字一致，老订阅链接不受影响。
...
- 支持只检测新代理，或强制检测全部代理。
- 支持 `stable_only`、`include_unstable`、`archive_all`、`target_only` 四种入库策略。声明过达标门槛的档位（如 `zai`）在 `stable_only` 下只收 `usable` 的线路：门禁不通过的线路哪怕评级是 A/B 也不会进仓库。`target_only` 更严格，只收 `usable=true` 的线路。
...
- 新增「附加探针 + 达标门槛」机制：档位可以声明额外探测目标（`aux_probes`）与必须每轮通过的门槛（`required_probes`）。既有 5 个档位两个字段都为空，行为完全不变。
- `zai` 档补上 ZCode 套餐入口 `zcode.z.ai`（ZCode 客户端真正访问的域名，而网页 chat.z.ai 只是消费端站点）。默认只打根路径，置 `ZAI_AUX_DEEP=1` 时改打无鉴权配置接口并校验响应特征，用于识别代理侧注入的占位页。
- `zai` 档以 `zcode.z.ai` 为门槛：它每轮都不通过时，线路即便服务与 API 全通也不再算有效，原因写进 `usable_reason`（例如「ZCode 入口 不可达(0/2)」）。`grade` 仍照常给出，供人工参考。
- 结果新增 `usable` / `usable_reason` / `aux_reachable` / `protocol_note` 字段；`checks_detail.targets` 会列出实际探测的目标，便于解释「到底打了哪些域名」。
- 入库策略新增 `target_only`（只收本档可用）；对声明了门槛的档位，`stable_only` 一律按 `usable` 收。未声明门槛的档位行为不变。
- 订阅链接（TXT / JSON）支持可选过滤参数：`grade`、`usable`、`stable`、`proto`、`exclude_socks4`、`exclude_shared_ip`、`limit`、`comment`。不带参数时输出与从前逐字一致。
- 仓库条目新增 `pass_streak` / `last_ok` / `fail_count`，写库时自动标注共享出口 `ip_shared`。
- 前端：结果与仓库新增「本档可用」「ZCode入口可达」「共享出口」「连续 N 轮」标签与对应筛选；自动任务入库策略下拉新增「只入库本档目标可用」；仓库链接弹窗新增导出过滤勾选，勾选后链接自动带上参数。
- 超时预算把附加探针计入，单条硬超时上限由 90 秒提到 120 秒（默认配置下 `zai` 由 74 秒变为 98 秒，其余档位不变）。
- 新增 `tools/test_repo_policy.py` 自检，覆盖入库口径、导出过滤与稳定性字段；`tools/test_zai_profile.py` 同步扩展附加探针与门槛断言。

## v6.4 到 v6.5 更新内容

- 页面、README 和 PRODUCT 版本号统一到 `v6.5`（此前正文停留在 `v6.3`，与实际发布线不一致）。
- 新增 `zai` 检测档位：检测 z.ai（会自动跟随跳转到 chat.z.ai）与 Z.AI API 域名（api.z.ai）。
- 判定标准基于实测：网页要求状态码正常且页面内含 `z.ai` / `chatglm` / `zhipu` 特征；API 域名沿用 401/403 视为“域名可达”的既有语义（未鉴权实测返回 401 + code 1001）。
- 该档位开启挑战页识别：z.ai 边缘是阿里云 ESA 而非 Cloudflare，因此经由代理看到挑战页属于代理侧拦截，不再误判为可达。
- 该档位在同一轮内会重试一次（其余档位不受影响，重试次数默认为 0），失败时会在错误信息后追加档位专属提示；单条代理的硬超时预算已把重试计入（该档 74 秒，其余 50 秒）。
- 新增 `tools/test_zai_profile.py` 自检脚本，覆盖判定标准、重试行为与既有档位的零回归。
- 容器化部署（`Dockerfile` / `docker-compose.yml` / GHCR 多架构镜像）在 v6.4 交付，说明见 [DOCKER.md](./DOCKER.md)。
